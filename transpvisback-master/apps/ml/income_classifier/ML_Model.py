...
class MLModel:
    ...
    def __init__(self): 
        ...
        with open(CURRENT_DIR + "/text-generator/model.json", 'rb') as json_file:
            self.loaded_model_json = json_file.read()
        json_file.close()
        self.in_tokenizer = pickle.load(open(CURRENT_DIR + "/text-generator/in_tokenizer.pkl", "rb"))
        self.tr_tokenizer = pickle.load(open(CURRENT_DIR + "/text-generator/tr_tokenizer.pkl", "rb"))
        self.loaded_model = model_from_json(self.loaded_model_json)
        self.loaded_model.load_weights(CURRENT_DIR + "/text-generator/model.h5")
